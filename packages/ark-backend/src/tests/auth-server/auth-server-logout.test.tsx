        ...
        .activate(({ use, useModule }) => {
          ...
          enableAuth({
            ...
            async isTokenBlacklisted(token) {
              const t = blacklistedTokens.find((t) => t.token === token);
              return Boolean(t);
            },
          });
          ...
          return puppeteer.launch().then((browser) => {
            return browser
              ...
              .then(async (page) => {
                ...
                await page.goto('http://localhost:3001/log-me-out');

                bodyHTML = await page.evaluate(() => document.body.innerHTML);

                expect(blacklistedTokens.length).toStrictEqual(1);
                expect(blacklistedTokens[0].token).toBeTruthy();
                expect(blacklistedTokens[0].issuedAt).toBeTruthy();

                await page.goto('http://localhost:3001');

                bodyHTML = await page.evaluate(() => document.body.innerHTML);

                expect(bodyHTML).toContain('Hello from main server: null');
              })
              .then(() => {
          ...
          return puppeteer.launch().then((browser) => {
            return browser
              ...
              .then(async (page) => {
                ...
                await page.goto('http://localhost:3000/log-me-out');
